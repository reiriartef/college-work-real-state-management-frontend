...
const ClientesScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    if (selectedOption === 'Mostrar por Tipo') {
      const data = await clientesBy(searchValue, 'tipo_cliente');
      setData(data);
    }
  };
  ...
  const todosLosClientes = async () => {
    try {
      return await fetch('http://127.0.0.1:5000/clientes').then((response) =>
        response.json()
      );
    } catch (error) {
      console.log(error);
  ...
  const clientesBy = async (value, param) => {
    try {
      return await fetch(`http://127.0.0.1:5000/clientes_by?${param}=${value}`).then(
    ...
    <div className="p-6">
      <nav className="bg-gray-200 rounded-lg p-4 mb-4">
        <div className="flex items-center justify-center space-x-4">
          {['Todos los clientes', 'Mostrar por nombre', 'Mostrar por Apellido', 'Mostrar por Tipo', 'Agregar nuevo Cliente'].map(
            (option) => (
              <button
                key={option}
      ...
      <div>
        {selectedOption === 'Agregar nuevo Cliente' ? (
              ...
              {data.length > 0 ? (
                data.map((cliente, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    <td className="py-2 px-4 text-left">{cliente.id_cliente}</td>
                    ...
                    <td className="py-2 px-4 text-left">{cliente.direccion}</td>
                    <td className="py-2 px-4 text-left">{cliente.tipo_cliente}</td>
                  </tr>
                ))
              ) : (
                <tr>
